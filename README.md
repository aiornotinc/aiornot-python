...
text. Optional CSV columns include `id` and `external_id`; `external_id` must be
36 characters or fewer. Image and video CSV batches can also include `only` and
`excluding` columns. Text CSV batches can include `include_annotations`.
